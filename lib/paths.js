...
async function symlink(appPath, libPath = "app.dev/lib") {
  if (!fs.existsSync(appPath)) {
    throw new Error(`App not found at: ${appPath}`);
  }
  const target = "/Users/serebano/dev/@clapp";
  const dest = import_path.default.resolve(appPath, "Contents/Resources", libPath);
  if (!fs.existsSync(dest)) {
    fs.mkdirSync(dest, { recursive: true });
  }
  const linkPath = import_path.default.resolve(dest, import_path.default.dirname(target));
